...
from networks.mmcv_custom import my_load_checkpoint as load_checkpoint
    ...
    def forward(self, x, H, W):
        ...
        if self.ca_attention == 1:
            ...
            x = (attn @ v).transpose(1, 2).reshape(B, N, C) + \
                self.dw_conv(v.transpose(1, 2).reshape(B, N, C).transpose(1, 2).view(B, C, H, W)).view(B, C,
                                                                                                       N).transpose(1,
                                                                                                                    2)
...
class SMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[4, 6, 28, 2], ca_attentions=[1, 1, 1, 0], num_stages=4, head_conv=7, expand_ratio=2, **kwargs):
        super().__init__()
    ...
    def init_weights(self, pretrained="mypretrain/smt_base.pth"):
...
def smt_t(pretrained=False, drop_path_rate=0., **kwargs):
    model = SMT(
        embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16],
        mlp_ratios=[4, 4, 4, 2],
        qkv_bias=True, depths=[2, 2, 8, 1], ca_attentions=[1, 1, 1, 0], head_conv=3, expand_ratio=2,
...
def smt_s(pretrained=False, drop_path_rate=0., **kwargs):
    model = SMT(
        embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16],
        mlp_ratios=[4, 4, 4, 2],
        qkv_bias=True, depths=[3, 4, 18, 2], ca_attentions=[1, 1, 1, 0], head_conv=3, expand_ratio=2,
        drop_path_rate=drop_path_rate, **kwargs)
    model.default_cfg = _cfg()
...
def smt_b(pretrained=False, drop_path_rate=0., **kwargs):
    model = SMT(
        embed_dims=[64, 128, 256, 512], ca_num_heads=[4, 4, 4, -1], sa_num_heads=[-1, -1, 8, 16],
        mlp_ratios=[8, 6, 4, 2],
        qkv_bias=True, depths=[4, 6, 28, 2], ca_attentions=[1, 1, 1, 0], head_conv=7, expand_ratio=2,
        drop_path_rate=drop_path_rate, **kwargs)
    model.default_cfg = _cfg()
...
def smt_l(pretrained=False, **kwargs):
    model = SMT(
        ...
        mlp_ratios=[8, 6, 4, 2],
        qkv_bias=True, depths=[4, 6, 28, 4], ca_attentions=[1, 1, 1, 0], head_conv=7, expand_ratio=2, drop_path_rate=0.,
        **kwargs)
    model.default_cfg = _cfg()
...
if __name__ == "__main__":
    ...
    tgt_img = torch.randn(B, 3, 640, 192).cuda()
    ...
    print(tgt_depth[-3].size())
    print(tgt_depth[-2].size())
    print(tgt_depth[-1].size())
