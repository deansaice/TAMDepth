...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        if opt.model == 'litemono':
            encoder = networks.MPMono(height=encoder_dict['height'],
                                      width=encoder_dict['width'])
        elif opt.model == "smtmono":
                    encoder = networks.smt_t()

        elif opt.model == "smtadapter":
    ...
    for i in range(pred_disps.shape[0]):
        ...
        if opt.eval_split == "eigen" or "eigen_benchmark":
