...
from networks.ops.modules import MSDeformAttn
...
def deform_inputs(x, a):
    ...
    reference_points = get_reference_points([(h // (4*a), w // (4*a))], x.device)
    deform_inputs1 = [reference_points, spatial_shapes, level_start_index]

    spatial_shapes = torch.as_tensor([(h // (4*a), w // (4*a))], dtype=torch.long, device=x.device)
    level_start_index = torch.cat((spatial_shapes.new_zeros(
        (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x2 = x[:, 16 * n:20 * n, :].transpose(1, 2).view(B, C, H // 4, W // 4).contiguous()
        x3 = x[:, 20 * n:, :].transpose(1, 2).view(B, C, H // 8, W // 8).contiguous()
...
class Conv33(nn.Module):
    def __init__(self, dim=768):
        super().__init__()
        self.conv = nn.Conv2d(dim, dim*2, 3, 1, 1, bias=False)
        self.norm = nn.BatchNorm2d(dim * 2)
        self.act = nn.ReLU()

    def forward(self, x, H, W, N):
        H, W = H * (2 ** N), W * (2 ** N)
        B, N, C = x.shape
        n = N // 21
        x1 = x[:, 0:16 * n, :].transpose(1, 2).view(B, C, H // 2, W // 2).contiguous()
        x2 = x[:, 16 * n:20 * n, :].transpose(1, 2).view(B, C, H // 4, W // 4).contiguous()
        x3 = x[:, 20 * n:, :].transpose(1, 2).view(B, C, H // 8, W // 8).contiguous()
        x1 = self.norm(self.conv(x1)).flatten(2).transpose(1, 2)
        x2 = self.norm(self.conv(x2)).flatten(2).transpose(1, 2)
        x3 = self.norm(self.conv(x3)).flatten(2).transpose(1, 2)
        x = torch.cat([x1, x2, x3], dim=1)
        x = self.act(x)
        return x
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W, N):
        ...
        if self.extra_extractors is not None:
            for extractor in self.extra_extractors:
                c = extractor(query=c, reference_points=deform_inputs2[0],
                              ...
                              level_start_index=deform_inputs2[2], H=H, W=W)
        self.num += 1
        print("interaction + {}".format(self.num))
        return x, c


class SpatialPriorModule(nn.Module):
    def __init__(self, inplanes=64, embed_dim=64):
        super().__init__()
